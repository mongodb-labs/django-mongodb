...
class AbstractIterableField(models.Field):
    ...
    def contribute_to_class(self, cls, name):
        ...
        if isinstance(self.item_field, models.ForeignKey) and isinstance(
            ...
            """
            If rel.to is a string because the actual class is not yet defined,
            look up the actual class later. Reference:
    ...
    #            def _resolve_lookup(resolved_model):
    #                self.item_field.remote_field.model = resolved_model
    #                self.item_field.do_related_class(self, cls)

    #            # add_lazy_relation(cls, self, self.item_field.rel.to, _resolve_lookup)
    #            lazy_related_operation(_resolve_lookup, self.item_field.remote_field.model)
